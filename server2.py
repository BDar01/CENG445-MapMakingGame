...
class GameServer:
    ...
    def serialize_objects_list(objects_list):
        return [{"id": obj[0], "type": obj[1], "x": obj[2], "y": obj[3]} for obj in objects_list]
    ...
    def send_notification(self, username, message, team, lock):
        if username in self.user_sockets:
            for u in self.user_factory.user_list:
                if u.username != username and u.player.team == team and u.username in self.user_sockets:
                    with lock:
                        user_socket = self.user_sockets[u.username]
                        user_socket.send(json.dumps({"Message": "Notify", "Map": {"objects": self.serialize_objects_list(message)}}).encode())
        ...
        try:
            if data['command'] == "listusers":
                ...
                if user.player:
                    ...
                    self.send_notification(user.username, player.map.teams[player.team].objects_list, player.team, lock)
                        ...
                        if data:
                            ...
                            if command == "L":
                                ...
                                if self.authenticate_user(username, password):
                                    thread_local.authenticated_user = user
                                ...
                                if (self.user_factory.user_list[user_id].checksession(token)):
                                    ...
                                    thread_local.authenticated_user = self.user_factory.user_list[user_id]
