...
def custom_logout(request, *args, **kwargs):
    if request.method == 'POST':
        # If it's a POST request, handle it with the default Django LogoutView
        return DjangoLogoutView.as_view()(request, *args, **kwargs)
    elif request.method == 'GET' and request.user.is_authenticated:
        # If it's a GET request and the user is authenticated, log them out
        from django.contrib.auth import logout
        logout(request)
        return render(request, 'registration/logout.html', {'logged_out': True, 'login_redirect_url': reverse_lazy(settings.LOGIN_REDIRECT_URL)})
    else:
...
def register(request, stid = None):
	...
	'''
	try:
		if request.user.has_perm('student.add_student'): # admin user
